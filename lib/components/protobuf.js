...
pro.getProtosRaw = function() {
    return {
        server : this.serverProtos_ || {},
        client : this.clientProtos_ || {},
...
pro.setProtos = function(type, path) {
  ...
  if(type === Constants.RESERVED.SERVER) {
    this.serverProtos_ = require(path);
    this.serverProtos = protobuf.parse(this.serverProtos_, true);
  }

  if(type === Constants.RESERVED.CLIENT) {
    this.clientProtos_ = require(path);
    this.clientProtos = protobuf.parse(this.clientProtos_);
  }
...
pro.onUpdate = function(type, path, event) {
  ...
  fs.readFile(path, 'utf8' ,function(err, data) {
    try {
      ...
      if(type === Constants.RESERVED.SERVER) {
        self.serverProtos_ = protos;
        self.serverProtos = protobuf.parse(protos, true);
        protobuf.setEncoderProtos(self.serverProtos);
      } else {
        self.clientProtos_ = protos;
        self.clientProtos = protobuf.parse(protos);
        protobuf.setDecoderProtos(self.clientProtos);
      }
